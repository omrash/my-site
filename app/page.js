import { useState, createHooks } from 'react'

const [useState, setState] = createHooks()
        ...
        <div className="flex flex-col items-center justify-center text-center">
          <div className="mb-8">
            <div className="w-32 h-32 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center text-white text-4xl font-bold">
              {localState?.name?.charAt(0)}
            </div>
          </div>
          
          <h1 className="text-5xl font-bold text-white mb-4">
            {localState?.name || 'Your Name'}
          </h1>
          
          <p className="text-xl text-purple-300 mb-2">
            {localState?.title || 'Software Engineer'}
          </p>
          
          <p className="text-lg text-gray-300 max-w-2xl mb-8">
            {localState?.bio || 'Building things, one commit at a time.'}
          </p>
          
          <div className="flex flex-wrap gap-2 mb-12">
            {(localState?.skills || []).map((skill, index) => (
              <span
                key={index}
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-4xl">
            {(localState?.projects || []).map((project, index) => (
              <div
                key={index}
